		...
		var AddFile = function(uploads, e, data) {
			...
			var RemoveFile = function(e) {
				...
				else
				{
					...
					if (settings.deleted)  settings.deleted.call(inforow, e, data.originalFiles.filter(function(ele){ 
			...
			if (settings.edit && !data.ff_info.errors.length)
			{
				inforow.find('.ff_fileupload_filename').append($('<input>').attr('type', 'text').attr('readonly', 'readonly').val(filename).keydown(function(e) {
